...
export class ServiceContainerBuilder {
    ...
    private resolveInjections(): void {
        for (const [token, service] of this.storage) {
            this.resolveInjection(token, service);
        }
    }

    public resolveInjection(token: InjectToken, service: unknown): void {
        ...
        Object.getOwnPropertyNames(service).forEach(p => {
            ...
            if (value instanceof InjectionPlace) {
                if (!value.takeAll) {
                    (service as Record<string, unknown>)[p] = this.storage.get(
                        value.type
                    );
                } else {
            ...
            } else if (value instanceof InjectionPredicate) {
                (service as Record<string, unknown>)[p] =
                    this.storage.getByPredicate(value.predicate);
            }
        });
    }
}
